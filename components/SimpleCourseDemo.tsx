...
// HARDCODED TOKENS - Žádný Supabase!
const VALID_TOKENS = {
...
export function SimpleCourseDemo() {
  ...
  // Check auth on mount - SIMPLE token check
  useEffect(() => {
    ...
    // Try token from URL
    if (token && VALID_TOKENS[token as keyof typeof VALID_TOKENS]) {
    ...
    // Try saved token
      ...
      if (savedToken && VALID_TOKENS[savedToken as keyof typeof VALID_TOKENS]) {
        const user = VALID_TOKENS[savedToken as keyof typeof VALID_TOKENS];
        setIsAuthenticated(true);
        setUserData(user);
        setAuthToken(savedToken);
        console.log("✅ Saved token valid:", user);
      } else {
