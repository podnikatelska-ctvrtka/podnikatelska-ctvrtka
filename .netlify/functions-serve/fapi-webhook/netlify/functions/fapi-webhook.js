...
var fapi_webhook_exports = {};
__export(fapi_webhook_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(fapi_webhook_exports);
...
async function sendEmail(to, subject, html) {
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: "Podnikatelsk\xE1 \u010Ctvrtka <kurz@podnikatelskactvrtka.cz>",
      to: [to],
      subject,
      html
    })
...
async function handler(event, context) {
  ...
  try {
    console.log("\u{1F3AF} FAPI webhook received - v2.0");
    console.log("\u{1F511} ENV check:", {
    ...
    console.log("\u{1F4CB} Invoice ID:", invoiceId);
    const fapiUsername = process.env.FAPI_USERNAME;
    const fapiApiKey = process.env.FAPI_API_KEY;
    console.log("\u{1F4DE} Fetching invoice from FAPI...");
    console.log("\u{1F511} Username exists:", !!fapiUsername);
    console.log("\u{1F511} API Key exists:", !!fapiApiKey);
    console.log("\u{1F310} Request URL:", `https://api.fapi.cz/invoices/${invoiceId}`);
    const authString = Buffer.from(`${fapiUsername}:${fapiApiKey}`).toString("base64");
    console.log("\u{1F510} Auth header prepared");
    ...
    console.log("\u{1F4E1} FAPI Response Status:", fapiResponse.status);
    console.log("\u{1F4E1} FAPI Response OK:", fapiResponse.ok);
    console.log("\u{1F4E1} FAPI Response Headers:", Object.fromEntries(fapiResponse.headers.entries()));
    if (!fapiResponse.ok) {
    ...
    console.log("\u{1F4C4} Invoice data:", JSON.stringify(invoice, null, 2));
    const email = invoice.customer?.email;
    const name = invoice.customer?.name || invoice.customer?.first_name || "Z\xE1kazn\xEDk";
    const amount = parseFloat(invoice.total || 0);
    if (!email) {
      throw new Error("No email in invoice.customer");
    }
    console.log("\u{1F464} Customer:", { email, name, amount });
    const accessToken = `${Date.now()}-${Math.random().toString(36).substring(2, 15)}-${Math.random().toString(36).substring(2, 15)}`;
    console.log("\u{1F4BE} Saving to Supabase...");
      ...
      <html>
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
          <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 40px 20px; text-align: center; border-radius: 10px 10px 0 0;">
            <h1 style="color: white; margin: 0; font-size: 28px;">\u{1F389} V\xEDtejte v kurzu!</h1>
          </div>
          ...
          <div style="background: white; padding: 40px 30px; border: 1px solid #e0e0e0; border-top: none; border-radius: 0 0 10px 10px;">
            ...
            <p>D\u011Bkujeme za zakoupen\xED kurzu <strong>Podnikatelsk\xE1 \u010Ctvrtka</strong>! \u{1F680}</p>
            
            <p>V\xE1\u0161 p\u0159\xEDstup do LMS syst\xE9mu je p\u0159ipraven:</p>
            
            <div style="background: #f8f9fa; padding: 20px; border-radius: 8px; margin: 30px 0;">
              <p style="margin: 0 0 15px 0; font-weight: 600; color: #667eea;">\u{1F511} V\xE1\u0161 p\u0159\xEDstupov\xFD odkaz:</p>
            ...
            <p style="color: #666; font-size: 14px;">
              \u{1F4A1} <strong>Tip:</strong> Ulo\u017Ete si tento email - odkaz funguje natrvalo a m\u016F\u017Eete se kdykoliv vr\xE1tit ke kurzu!
            </p>
            
            <hr style="border: none; border-top: 1px solid #e0e0e0; margin: 30px 0;">
            
            <p style="color: #999; font-size: 13px; margin-bottom: 0;">
              Pokud m\xE1te jak\xE9koliv dotazy, nev\xE1hejte odpov\u011Bd\u011Bt na tento email.<br>
              P\u0159ejeme hodn\u011B \xFAsp\u011Bch\u016F! \u{1F4AA}
            </p>
          </div>
        </body>
      </html>
    ...
    await sendEmail(
      email,
      "\u{1F389} P\u0159\xEDstup do kurzu Podnikatelsk\xE1 \u010Ctvrtka",
      emailHtml
    );
...
//# sourceMappingURL=fapi-webhook.js.map
